...
function is_VipOverdue($uid){
    $vip = M('vip')->field('vip_name,Registration_time,due_time,p_id,status')->where("u_id = $uid and status = 1")->find();//查看会员是否过期
    $day = time();
    $d_t = $vip['due_time'];
    ...
    if($d_t <= $day){
        ...
        $svip['status'] = 2;          //会员过期
        $pri = $vip['p_id'];
        $s_vip = M('vip')->where("u_id = $uid")->save($svip);

        $pri = M('user_group_link')->where("uid = $uid and user_group_id = $pri")->delete();
    }
 ...
 * 内部消息推送
 *
 * $uid  推送给用户的uid $content 推送内容
 */
function getMessagePush($uid,$content){

    $ss['min_max'] = '2_'.$uid;
    $mms = M('message_list')->field('list_id')->where($ss)->find();
    if(!$mms){
        $ss['from_uid'] = 2;
        $ss['type'] = 1;
        $ss['member_num'] = 2;
        ...
        M('message_list')->add($ss);

        $mms = M('message_list')->field('list_id')->where($ss)->find();

        $sss['list_id'] = $mms['list_id'];
        $sss['member_uid'] = $uid;
        $sss['message_num'] = 1;
        $sss['new'] = 1;
        $sss['ctime'] = time();
        M('message_member')->add($sss);

        $aa['list_id'] = $mms['list_id'];
        $aa['from_uid'] = 2;
        $aa['type'] = 'text';
        $aa['mtime'] = time();
        $aa['content'] = $content;
        M('message_content')->add($aa);
        $con = M('message_content')->field('message_id')->where($aa)->find();
        ...
        $last_message['last_message'] = 'a:6:{s:8:"from_uid";i:2;s:4:"type";s:4:"text";s:7:"list_id";i:'.$mms['list_id'].';s:5:"mtime";i:'.time().';s:7:"content";s:15:"消息通知！";s:10:"message_id";s:3:"'.$con['message_id'].'";}';

        M('message_list')->where("list_id={$mms['list_id']}")->save($last_message);

    }else{
        $sss['list_id'] = $mms['list_id'];
        $sss['member_uid'] = $uid;
        $num1 = M('message_member')->field('message_num,new')->where($sss)->find();
        $num = $num1['message_num'] + 1;
        $new = $num1['new'] +1;
        $data['message_num'] = $num;
        $data['new'] = $new;
        M('message_member')->where($sss)->save($data);
