...
// 			$authenticate = array();
    ...
    public function getUserTodayIncome($uid,$creditType)
    {
        if (!$uid) return 0;
        return 0;
    }

    public function isPhone($uid,$phone)
    {
        $user = $this->field('phone')->where("uid={$uid} and phone={$phone}")->find();
        if($user){
            return true;
        }
        return false;
    ...
    public function isPhoneNull($uid,$phone)
    {
        $user = $this->field('phone')->where("uid={$uid} and phone={$phone}")->find();
        if($user['phone'] == 'null'){
            return true;
