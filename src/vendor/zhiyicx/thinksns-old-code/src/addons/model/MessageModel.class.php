...
/*    		$body = 'Hi , <a href="'.U('public/Profile/index',array('uid'=>$v['uid'])).'" style="color:#0000ff;text-decoration:none;">'.$v['uname'].'</a><br /><br />自从您上次登录<a style="text-decoration:none;color:#3366cc" href="'.SITE_URL.'">'.
    ...
    //as
    public function get_message_top_status($list_id,$user_id)
    {
       
        $map['list_id'] = $list_id;
        $map['uid'] = $user_id;
      
        $info = M("message_top")->where($map)->find();
        if ($info){
            return true;
        }else{
            return false;
    ...
    //
    public function _set_message_top($list_id,$user_id){
        $topParam['list_id']=$list_id;
        $topParam['uid']=$user_id;
        $top_id = M('message_top')->data($topParam)->add();

        return $top_id;
    }
    
    public function _remove_message_top($list_id,$user_id){
        $topParam['list_id']=$list_id;
        $topParam['uid']=$user_id;
        $top_id = M('message_top')->where($topParam)->delete();
        return $top_id;
    }
     ...
     * @return array 私信列表数据
     */
    public function getTopMessageListByUidForAPI($uid, $type = 1, $since_id = 0, $max_id = 0, $count = 20, $page = 1, $order = '`mb`.`new` DESC,`mb`.`list_id` DESC')
    {
        $uid = intval($uid);
        $type = is_array($type) ? ' IN ('.implode(',', $type).')' : "={$type}";
        $where = "`mb`.`member_uid`={$uid} AND `li`.`type`{$type} AND mb.is_del = 0 AND mb.message_num > 0";
        ...
        //as
        $where .= "  AND `top`.`uid`={$uid}";
        $limit = ($page - 1) * $count.','.$count;
        $list = D('message_member')->table("`{$this->tablePrefix}message_member` AS `mb`")
        ->join("`{$this->tablePrefix}message_list` AS `li` ON `mb`.`list_id`=`li`.`list_id`")
        ->join("`{$this->tablePrefix}message_top` AS `top` ON `mb`.`list_id`=`top`.`list_id`")
        ->where($where)
        ->order('`mb`.`list_ctime` DESC')
        ->limit($limit)
        ->findAll();
