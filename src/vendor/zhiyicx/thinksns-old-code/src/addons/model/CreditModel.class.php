...
class CreditModel extends Model
{
    ...
    public function getUserUbank($uid,$type)
    {   
        if($type == 1){
            return M('credit_user')->field('ubank')->where("uid={$uid}")->find()['ubank'];
        }elseif ($type == 2) {
            return M('credit_user')->field('caiyuanbi')->where("uid={$uid}")->find()['caiyuanbi'];
        }
