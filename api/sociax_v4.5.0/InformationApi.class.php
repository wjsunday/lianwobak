...
';
    ...
    public function share(){
      $id[id] = $this->data['id'];//资讯id
      $subjects = M('information_list')->field('id,content,subject,abstract')->where($id)->find();
      if(!$subjects){
        $res = array();
        $res['msg'] = '没有找到资讯';
        $res['status'] = 0;
      ...
      preg_match_all('/\<img(.*?)src\=\"(.*?)\"(.*?)\/?\>/is', $subjects['content'], $image);
      $subjects['image'] = $image[2];
      if ($subjects['image'] && is_array($subjects['image']) && count($subjects['image']) >= 1) {
          $subjects['image'] = $subjects['image'][array_rand($subjects['image'])];
          if (!preg_match('/https?\:\/\//is', $subjects['image'])) {
              $subjects['image'] = parse_url(SITE_URL, PHP_URL_SCHEME).'://'.parse_url(SITE_URL, PHP_URL_HOST).'/'.$subjects['image'];
          }
      }
      unset($subjects['content']);
