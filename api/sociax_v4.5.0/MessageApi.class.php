...
'comment' => 0, 'atme' => 0, 'digg' => 0, 'follower' => 0, 'weiba' => 0, 'weiba_comment' => 0, 'unread_digg_weibapost' => 0,
        ...
        $sum = M('user_follow_num')->field('num')->where("uid={$this->mid}")->find();
        $count = model('UserData')->setUid($GLOBALS['ts']['mid'])->getUserData();

        //判断会员是否过期
        model('VipPay')->is_VipOverdue($this->mid);
        model('Cyb')->cybOverdue($this->mid);
        Bonus::unclaimedBonusReturn($this->mid);
        return array(
            ...
            'friend'                => (string) intval($sum['num']),
            'isVip'                 => model('VipPay')->isVip($this->mid),
            'isPersonalVip'         => model('VipPay')->isPersonalVip($this->mid),
            'isBusinessVip'         => model('VipPay')->isBusinessVip($this->mid),
            'todayIncome'           => model('UserData')->todayIncome(),
        );
    }
    ...
    {
        if (!$this->data['list_id']) {
            $this->error(array(
                'status' => '0',
                'msg'    => '系统参数错误!',
            ));
        }
        $status = D("message")->get_message_top_status($this->data['list_id'],$this->mid);
        
        if ( !$status)
        {    
           
            D("message")->_set_message_top($this->data['list_id'],$this->mid);
        }
        $this->success(array(
            'status' => '1',
            'msg'    => '置顶成功',
    ...
    public function remove_top_message(){
        ...
        if ( $status)
        {        
            D("message")->_remove_message_top($this->data['list_id'],$this->mid);
        }
     ...
     * @return array
     */
    public function get_top_message_list()
    {
        $this->data['type'] = $this->data['type'] ? $this->data['type'] : array(1, 2);
        $this->data['order'] = $this->data['order'] == 'ASC' ? '`list_ctime` ASC' : '`list_ctime` DESC';
        $message = model('Message')->getTopMessageListByUidForAPI($this->mid, $this->data['type']);
        $message = $this->__formatMessageList($message);
