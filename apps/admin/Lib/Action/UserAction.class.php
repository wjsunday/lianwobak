            ...
            if ($status == 1) {
                ...
                if (is_array($id)) {
                    foreach ($id as $k => $v) {
                        ...
                        $title = M('user_verified_category')->field('title')->where("user_verified_category_id={$user_group['user_verified_category_id']}")->find();
                        if ($exist) {
                            continue;
                        }
                        ...
                        if($user_group['usergroup_id'] == 5){
                            $data_list = array(
                                array('title'=>'认证类型','content'=>'个人认证('.$title['title'].')'),
                                array('title'=>'姓名','content'=>$user_group['realname']),
                                array('title'=>'身份证','content'=> substr($user_group['idcard'], 0, 4).'****'.substr($user_group['idcard'], -4)),
                                array('title'=>'手机号','content'=>substr($user_group['phone'], 0, 3).'****'.substr($user_group['phone'], -4)),
                                array('title'=>'审核人','content'=>getUserName($this->mid)),
                                array('title'=>'通过时间','content'=>time()),
                            );
                            $type = 1;
                        }elseif ($user_group['usergroup_id'] == 6) {
                            $data_list = array(
                                array('title'=>'认证类型','content'=>'商家认证('.$title['title'].')'),
                                array('title'=>'商家名称','content'=>$user_group['company']),
                                array('title'=>'社会信用号','content'=> substr($user_group['credit_code_number'], 0, 4).'****'.substr($user_group['credit_code_number'], -4)),
                                array('title'=>'经营地址','content'=> $user_group['business_address']),
                                array('title'=>'运营者','content'=>$user_group['realname']),
                                array('title'=>'身份证','content'=> substr($user_group['idcard'], 0, 4).'****'.substr($user_group['idcard'], -4)),
                                array('title'=>'审核人','content'=>getUserName($this->mid)),
                                array('title'=>'通过时间','content'=>time()),
                            );
                        ...
                        model('Jpush')->authenticationMessage($user_group['uid'], '认证通知','认证成功',$type,$data_list);
                    ...
                    foreach ($id as $k => $v) {
                        ...
                        if($user_group['usergroup_id'] == 5){
                            $data_list = array(
                                ...
                                array('title'=>'手机号','content'=>substr($user_group['phone'], 0, 3).'****'.substr($user_group['phone'], -4)),
                                array('title'=>'审核人','content'=>getUserName($this->mid)),
                                array('title'=>'驳回理由','content'=>$rejectInfo),
                            );
                            $type = 1;
                        }elseif ($user_group['usergroup_id'] == 6) {
                            $data_list = array(
                        ...
                        model('Jpush')->authenticationMessage($user_group['uid'], '认证通知','认证失败',$type,$data_list);
                        model('User')->cleanCache($user_group['uid']);
                        unset($user_group);
                        unset($maps);
                    }
