...
class UserGroupAction extends AdministratorAction
{
    ...
    public function index()
    {
         ...
         // 页面具有的字段，可以移动到配置文件中！！！
        $this->pageKeyList = array('user_group_id', 'app_name', 'user_group_name', 'user_group_type', 'user_group_icon', 'is_authenticate', 'is_vip', 'is_business_vip', 'DOACTION');
        ...
        foreach ($list['data'] as &$value) {
            ...
            $value['is_vip'] = $value['is_vip'] == 1 ? '是' : '否';
            $value['is_business_vip'] = $value['is_business_vip'] == 1 ? '是' : '否';
            ...
            if ($value['user_group_id'] > 6 && $value['user_group_id'] != 7 && $value['user_group_id'] != 8 && $value['user_group_id'] != 11 && $value['user_group_id'] != 12) {
    ...
    public function addUsergroup()
    {
        ...
        $this->pageKeyList = array('user_group_id', 'user_group_name', 'user_group_icon', 'user_group_type', 'is_authenticate', 'is_vip', 'is_business_vip');

        $this->opt['user_group_type'] = array(0 => L('PUBLIC_ORDINARY'), 1 => L('PUBLIC_SPECIAL'));
        $this->opt['is_authenticate'] = array(1 => '是', 0 => '否');
        $this->opt['is_vip'] = array(1 => '是', 0 => '否');
        $this->opt['is_business_vip'] = array(1 => '是', 0 => '否');
