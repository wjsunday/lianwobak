...
class ApplicationAction extends AdministratorAction
{
    ...
    public function ZB_credit_order()
    {
        ...
        foreach ($list['data'] as $key => &$value) {
            ...
            switch ($value['type']) {
                case '1':
                    $value['type'] = '<font color="">支付宝</font>';
                    break;
                case '2':
                    $value['type'] = '<font color="">微信</font>';
                    break;
                case '3':
                    $value['type'] = '<font color="">银行卡</font>';
                    break;
            }
    ...
    //绑定
    public function ZB_binding()
    {
        $this->pageTab[] = array('title' => '提现记录', 'tabHash' => 'ZB_credit_order', 'url' => U('admin/Application/ZB_credit_order'));
        $this->pageTab[] = array('title' => '绑定记录', 'tabHash' => 'ZB_binding', 'url' => U('admin/Application/ZB_binding')); 
        $this->pageButton[] = array('title' => '搜索记录', 'onclick' => "admin.fold('search_form')");
        $this->pageKeyList = array('uid', 'name', 'type', 'account', 'bank', 'phone', 'address', 'ctime');
        $this->searchKey = array('uid', 'account', 'type');
        $this->$searchPostUrl = U('admin/Application/ZB_binding');
        $this->_listpk = 'account';
        if ($_POST) {
            $_POST['uid'] && $map['uid'] = array('in',$_POST['uid']);
            $_POST['type'] && $map['type'] = trim(intval($_POST['type']));
            $_POST['account'] && $map['account'] = array('like', '%'.$_POST['account'].'%');
        }
        $list = D('user_account')->where($map)->order('ctime desc')->findPage(20);
        foreach ($list['data'] as $key => &$value) {
            ...
            $value['ctime'] = date('Y年m月', $value['ctime']);
        }
        $this->displayList($list);
    }
    ...
    private function solveOrder($number, $type, $reason = '')
    {
        ...
        if ($orderinfo->status == 0) {
            ...
            $shouxu = floatval($orderinfo->gold)*0.01;
            if ($do) {
                $uinfo = D('User')->where(array('uid' => $orderinfo->uid))->find();
                if ($type == 1) {
                    $data_list = array(
                        array('title'=>'支付方式','content'=>'零用钱'),
                        array('title'=>'收款方式','content'=>$pay_type),
                        array('title'=>'手续费','content'=>floatval($shouxu)),
                        array('title'=>'交易方式','content'=>'链我支付密码'),
                        array('title'=>'提现时间','content'=>date('Y-m-d H:i:s',time())),
                        array('title'=>'备注','content'=>'您的提现申请已被处理，请注意查收'),
                    );
                    model('Jpush')->noticeMessage($orderinfo->uid,'提现通知','提现通知',$number,3,4,floatval($orderinfo->amount),$data_list);
                    ...
                    $data_list = array(
                        array('title'=>'支付方式','content'=>'零用钱'),
                        array('title'=>'收款方式','content'=>$pay_type),
                        array('title'=>'手续费','content'=>$shouxu),
                        array('title'=>'交易方式','content'=>'链我支付密码'),
                        array('title'=>'提现时间','content'=>date('Y-m-d H:i:s',time())),
                        array('title'=>'备注','content'=>'您的提现申请已被驳回，理由是'.$reason),
                    );
                    model('Jpush')->noticeMessage($orderinfo->uid,'提现通知','提现通知',$number,3,4,floatval($orderinfo->amount),$data_list);
                    $record['serial_number'] = date('YmdHis').rand(1000, 9999);
                    $record['cid'] = 0; //没有对应的积分规则
                    $record['type'] = 3; //5-提现
                    $record['uid'] = $orderinfo->uid;
                    $record['action'] = '提现驳回';
                    $record['des'] = '';
                    $record['change'] = $orderinfo->amount; 
                    $record['ctime'] = time();
                    $record['detail'] = json_encode(array('ubank' => '+'.$orderinfo->amount));
                    $record['reason'] = $reason;
                    D('credit_record')->add($record);
                    D('credit_user')->setInc('ubank', 'uid='.$orderinfo->uid, $orderinfo->gold);
                }
