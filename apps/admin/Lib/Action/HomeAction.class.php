...
class HomeAction extends AdministratorAction
{
    ...
    public function dosendmsg()
    {
        ...
        if($_POST['user_group_id'] == 0){
            $uid = M('user')->field('uid')->select();
            foreach ($uid as $key => $value) {
                $uid[$key] = $value['uid'];
            }
            $uid = implode(',',$uid);
        }else{
            $uid = M('user_group_link')->field('uid')->where("user_group_id={$_POST['user_group_id']}")->select();
            if(!$uid){
                $this->error('该用户组没有用户');
            }
            foreach ($uid as $key => $value) {
        ...
        // $this->error();
        $uids = explode(',', $uid);
        if(model('Jpush')->pushMessage($uids,$checkContents)){
            $this->success();
        }
        $this->error();
    }
