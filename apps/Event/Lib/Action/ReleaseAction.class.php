...
class Release extends Controller
{
    ...
    public function index()
    {
        ...
        //参与活动权限分类
        $pri = M('user_group')->field('user_group_id,user_group_name')->where('user_group_id > 6')->select();
        // var_dump($pri);exit;
    ...
    {
        list($title, $sdate, $stime, $edate, $etime, $area, $city, $address, $place, $image, $mainNumber, $price, $tips, $cate, $audit, $content,$num) = Common::getInput(array('title', 'sdate', 'stime', 'edate', 'etime', 'area', 'city', 'address', 'place', 'image', 'mainNumber', 'price', 'tips', 'cate', 'audit', 'content','num'), 'post');
        $audit != 1 and
        $audit = 0;
