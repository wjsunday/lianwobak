...
use Apps\Bonusevent\Model\Enrollment;
use Apps\Bonusevent\Model\Event;
use Apps\Bonusevent\Model\Star;

/**
 * 管理控制器
 *
 * @package Apps\Bonusevent\Controller\Manage
...
class Manage extends Controller
{
    ...
    {
        $eid = Common::getInput('eid', 'get');
        if (Bonusevent::getInstance()->setId($eid)->setUid($this->mid)->over()) {
            $this->success('操作成功！');
        }
        $this->error(Bonusevent::getInstance()->getError());
    }
    ...
    {
        $eid = Common::getInput('eid', 'get');
        if (Bonusevent::getInstance()->setId($eid)->setUid($this->mid)->delete()) {
            $this->assign('jumpUrl', U('Bonusevent/Index/index'));
            $this->success('删除成功！');
        }
        $this->error(Bonusevent::getInstance()->getError());
    }
    ...
    public function index()
    {
        ...
        switch ($type) {
            ...
            case 1:
                $list = Bonusevent::getInstance()->getMyEvent($this->mid);
                break;
...
class_alias('Apps\Bonusevent\Controller\Manage', 'ManageAction');
