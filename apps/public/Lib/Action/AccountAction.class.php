 ...
 * @version TS3.0
 */
use apps\Common\Extend\Pay\Alipay;
use apps\Common\Extend\Pay\Alipay\wappay\buildermodel\AlipayTradeWapPayContentBuilder;
use apps\Common\Extend\Pay\Alipay\wappay\service\AlipayTradeService;
class AccountAction extends Action
{
    ...
    public function vip(){
        $uid = $this->mid;
        is_VipOverdue($uid);
        $pri = M('user_group')->join('ts_vip_price ON ts_user_group.user_group_id = ts_vip_price.u_g_id')->where('is_vip = 1')->field('user_group_name,user_group_id,vip_price')->select();
        
        $vip = M('vip')->field('vip_name,Registration_time,due_time,p_id,status')->where("u_id = $uid and status = 1")->find();
        $aa = M('vip')->where('status!=1 AND u_time<'.(time() - (60 * 1)))->delete();

        $day = time();
        $d_t = $vip['due_time'];
        // $Days=round(($ntime-$time)/3600/24);获取有效时间函数 floor返回不大于 value 的下一个整数，ceil 返回不小于 value 的下一个整数
        $dd = floor(round($d_t - $day)/3600/24);
        // var_dump($dd);exit;
        $this->assign('pri',$pri);
        $this->assign('vip',$vip);
        $this->assign('dd',$dd);
...
//续费
    public function vip_renew(){
        $uid = $this->mid;
        $vip = M('vip')->field('p_id')->where("u_id = $uid and status = 1")->find();
        $gid = $vip['p_id'];
        $pri = M('user_group')->join('ts_vip_price ON ts_user_group.user_group_id = ts_vip_price.u_g_id')->where("user_group_id = $gid")->field('user_group_name,user_group_id,vip_price')->find();
        // var_dump($pri);exit;
        $this->assign('pri',$pri);
        $this->display();
    }
    ...
    public function vip_pay(){
        ...
        $dd['serial_number'] = time().rand(1,9).rand(1,9).rand(1,9).rand(1,9);
        $dd['u_id'] = $this->mid;
        $dd['vip_name'] = $_POST['u_name'];
        $dd['vip_Price'] = $_POST['total'];
        $dd['p_id'] = $_POST['pid'];
        $dd['number'] = $_POST['num'];
        $dd['time_sta'] = $_POST['time'];
        $dd['charge_type'] = $_POST['charge_type'];
        $dd['status'] = 0;
        $dd['u_time'] = time();
        ...
        if($dd['charge_type'] == 0){
            ...
            if ($result) {
                ...
                $res['info'] = 'OK';
                switch ($type) {
                    case 0: $res['request_url'] = $this->alipay($dd); break;
                    case 1: $res['request_url'] = $this->weixin($dd); break;
        ...
        }elseif ($dd['charge_type'] == 2) {
            $dd['url'] = U('public/Account/pay_paw');
            $this->ajaxReturn($dd);
        }
   ...
   public function pay_paw(){

        // var_dump($_POST);exit;

        $paw = md5(trim($_POST['paw']));
        $uid = $this->mid;
        $user_paw = M('user')->field('pay_paw')->where("uid = $uid")->find();
        // var_dump($paw,$user_paw);exit;
        $u_paw = $user_paw['pay_paw'];
        if($paw == $u_paw){

            $dd['serial_number'] = trim($_POST['serial_number']);
            $dd['u_id'] = $this->mid;
            $dd['vip_name'] = trim($_POST['vip_name']);
            $dd['vip_Price'] = trim($_POST['vip_Price']);
            $dd['p_id'] = trim($_POST['p_id']);
            $dd['number'] = trim($_POST['number']);
            $dd['time_sta'] = trim($_POST['time_sta']);
            $dd['charge_type'] = trim($_POST['charge_type']);
            $dd['status'] = 1;
            $dd['u_time'] = $_POST['u_time'];
            $dd['Registration_time'] = time();
            $num = $dd['number'];
            if($dd['time_sta'] == 1){

                $mon = date("Y-m-d H:i:s",strtotime("+$num month"));
            }else if($dd['time_sta'] == 2){
                $num = 3*$num;
                $mon = date("Y-m-d H:i:s",strtotime("+$num month"));
            }else{

                $mon = date("Y-m-d H:i:s",strtotime("+$num year"));
            }

            $dd['due_time'] = strtotime($mon);
            // var_dump($dd);exit;
            $ubank = M('credit_user')->field('ubank')->where("uid = $uid")->find();
            if($dd['vip_Price'] > $ubank['ubank']){
                $this->ajaxReturn(-2);//账户余额不足
            }

            $ubank['ubank'] = $ubank['ubank'] - $dd['vip_Price'];
            $s_ubank = M('credit_user')->where("uid = $uid")->save($ubank);
            // var_dump($ubank);exit;

            $vip = M('vip')->add($dd);
            $pri['user_group_id'] = $dd['p_id'];
            $pri['uid'] = $uid;
            //赋予权限
            $pri = M('user_group_link')->add($pri);
            
            
            if($vip && $pri){
                $con = '恭喜您成功开通'.$dd['vip_name'].'会员';
                getMessagePush($dd['u_id'],$con);
                $this->ajaxReturn(1);
            }else{
    ...
    public function vip_pay_ren(){
        ...
        if($dd['charge_type'] == 2){
            $dd['url'] = U('public/Account/pay_ren');
            $this->ajaxReturn($dd);
        }
   ...
   public function pay_ren(){
        ...
        if($paw == $u_paw){
            // var_dump($_POST);
            $re_vip = M('vip')->field('serial_number,vip_Price,number,due_time')->where("u_id = $uid")->find();
            // var_dump($re_vip);
            $order = $re_vip['serial_number'];
            $dd['vip_Price'] = trim($_POST['vip_Price']) + $re_vip['vip_Price'];
            $dd['number'] = trim($_POST['number']) + $re_vip['number'];
            $dd['time_sta'] = trim($_POST['time_sta']);
            $dd['charge_type'] = trim($_POST['charge_type']);
            $dd['status'] = 1;
            $dd['u_time'] = $_POST['u_time'];
            $tt = $re_vip['due_time'];
            $now = time();
            // $dd['Registration_time'] = $re_vip['due_time'];
            ...
            $ubank = M('credit_user')->field('ubank')->where("uid = $uid")->find();
            if(trim($_POST['vip_Price']) > $ubank['ubank']){
                $this->ajaxReturn(-2);//账户余额不足
            }

            $ubank['ubank'] = $ubank['ubank'] - trim($_POST['vip_Price']);
            $s_ubank = M('credit_user')->where("uid = $uid")->save($ubank);
            // var_dump($ubank);exit;

            $vip = M('vip')->where("serial_number = $order")->save($dd);
    ...
    public function scoredetail()
    {
        ...
        // var_dump($credit_record);exit;
        $this->assign('credit_record', $credit_record);
        $this->display();
    }

//零用钱
    public function ubank()
    {   
        $user_info = model('User')->getUserInfo($this->mid);
        $this->assign('user_info', $user_info);
        $userCredit = model('Credit')->getUserCredit($this->mid);
        ...
        $ubank = D('credit_record_ubank')->field('oid,des,ctime,ubank_change,order_num')->where($map)->order('ctime DESC')->findPage(10);
        // var_dump($ubank);exit;
        $this->assign('ubank', $ubank);
        $this->display();
    }

    public function scorecharge_ubank()
    {
        // 删除7天前还没支付的记录
        $aa = D('credit_charge')->where('status=0 AND ctime<'.(time() - (86400 * 7)));
        $data = model('Xdata')->get('admin_Config:charge');
        $charge_record = D('credit_charge')->where('sta=1 and status>0 and uid='.$this->mid)->order('charge_id desc')->findPage(100);
        $this->assign('chargeConfigs', $data);
        $this->assign('charge_record', $charge_record);
        $this->display();
    }

    public function scoretransfer_ubank()
    {
        if ($_POST) {

            $_POST['fromUid'] = $this->mid;
            $result = model('Credit')->startTransfer_ubank();
            // var_dump($result);exit;
            if ($result) {

                $con = getUserName($this->mid).'向您转了'.$_POST['num'].'零用钱';
                getMessagePush($_POST['toUid'],$con);
                $this->success('转账成功！');
        ...
        $map['oid'] = $this->mid;
        $map['state'] = 1;
        $ubank = D('credit_record_ubank')->field('uid,des,ctime,ubank_change,order_num')->where($map)->order('ctime DESC')->findPage(10);

        $this->assign('ubank', $ubank);
        $this->display();
    }

    public function ubank_scorecharge()
    {
    ...
    {   
        ...
        $cyb = D('credit_record_ubank')->field('oid,des,ctime,cyb_change,order_num')->where($map)->order('ctime DESC')->findPage(10);
        // var_dump($cyb);exit;
        $this->assign('cyb', $cyb);
        $this->display();
    }

    public function scorecharge_cyb()
    {
        // 删除7天前还没支付的记录
        $aa = D('credit_charge')->where('status=0 AND ctime<'.(time() - (86400 * 7)));
        $data = model('Xdata')->get('admin_Config:charge');
        $charge_record = D('credit_charge')->where('sta=3 and status>0 and uid='.$this->mid)->order('charge_id desc')->findPage(100);
        $this->assign('chargeConfigs', $data);
        $this->assign('charge_record', $charge_record);
        $this->display();
    }

    public function scoretransfer_cyb()
    {
        if ($_POST) {
            $_POST['fromUid'] = $this->mid;
            $touid['uid'] = $_POST['toUid'];
            
            $result = model('Credit')->startTransfer_cyb();
            // var_dump($result);exit;
            if ($result) {
                $con = getUserName($this->mid).'向您转了'.$_POST['num'].'财源币';
                getMessagePush($_POST['toUid'],$con);
        ...
        $map['oid'] = $this->mid;
        $map['state'] = 2;
        $cyb = D('credit_record_ubank')->field('uid,des,ctime,cyb_change,order_num')->where($map)->order('ctime DESC')->findPage(10);
...
//财源币充值
    public function cyb_scorecharge()
    {
        $price = intval($_POST['charge_value']);
        if ($price < 1) {
            exit(json_encode(array('status' => 0, 'info' => '充值金额不正确')));
        ...
        if($type !=2){
            ...
            $data['status'] = 0;
            $data['sta'] = 3;
            $data['charge_cyb'] = intval($price*10);
            $data['charge_order'] = '';
            // var_dump($data);exit;
            $result = D('credit_charge')->add($data);
            $res = array();
            ...
            if($paw == $u_paw){
                ...
                $ubank = M('credit_user')->field('ubank,caiyuanbi')->where("uid = $uid")->find();
                // var_dump($ubank);exit;
                $uu = $ubank['ubank'];
                $cc = $ubank['caiyuanbi'];
                ...
                if($uu < $price){
                    ...
                    exit(json_encode(array('status' => 0, 'info' => '余额不足，请选择其他方式充值')));
                }
                // var_dump($data);exit;
                $cyb = M('credit_charge')->add($data);
                if($cyb){

                    $credit = M('credit_user')->where("uid={$data['uid']}")->save(array('ubank' => $uu - $data['charge_value'],'caiyuanbi' => $cc + $data['charge_cyb']));
                    if($credit){
                        exit(json_encode(array('status' => 1, 'info' => '充值成功','jumpUrl' => U('public/Account/scorecharge_cyb'))));
                    }else{
                        exit(json_encode(array('status' => 0, 'info' => '充值失败','jumpUrl' => U('public/Account/scorecharge_cyb'))));
                    }
                }else{
                        exit(json_encode(array('status' => 0, 'info' => '充值失败','jumpUrl' => U('public/Account/scorecharge_cyb'))));
                }
                ...
                exit(json_encode(array('status' => 0, 'info' => '密码不正确')));
            }
...
//         $id = $_GET['id'];
//         $con = M('credit_record_cyb')->field('oid,use_sta,cyb_change')->find($id);
//         $credit_user = M('credit_user')->where("uid={$con['oid']}")->field('caiyuanbi')->find();
//         // var_dump($credit_user);exit;
//         if($con['use_sta'] == 2){

//             $this->error('该证券已使用完');
//         }else if($con['use_sta'] == 3){

//             $this->error('该证券已过期');
//         }

//         $con1 = M('credit_user')->where("uid={$con['oid']}")->save(array('caiyuanbi'=>$credit_user['caiyuanbi'] + $con['cyb_change']));
//         $use_end = M('credit_record_cyb')->where("id = $id")->save(array('use_sta'=>2));
//         if($con1 && $use_end){
//             $this->success('使用成功');
//         }
...
//     }
    ...
    public function scorecharge()
    {
        ...
        $aa = D('credit_charge')->where('status=0 AND ctime<'.(time() - (86400 * 7)));
        $data = model('Xdata')->get('admin_Config:charge');
        $charge_record = D('credit_charge')->where('sta=2 and status>0 and uid='.$this->mid)->order('charge_id desc')->findPage(100);
        $this->assign('chargeConfigs', $data);
        $this->assign('charge_record', $charge_record);
        $this->display();
    }
    ...
    public function alipay(array $data)
    {
        ...
        if($_GET['act'] == 'do_scorecharge'){
        ...
        }elseif ($_GET['act'] == 'ubank_scorecharge') {
            ...
            'app'      => 'public',
            'mod'      => 'Account',
            'act'      => 'scorecharge_ubank',
            );
        }elseif ($_GET['act'] == 'cyb_scorecharge') {
        ...
        $config = $aliPayObj->getConfig();
        $config['notify_url'] =  SITE_URL.'/alipay_notify.php';
        $config['return_url'] =  SITE_URL.'/alipay_return.php';
        
        $payRequestBuilder = new AlipayTradeWapPayContentBuilder();
        $payRequestBuilder->setBody("充值");
        $payRequestBuilder->setSubject("测试");
        $payRequestBuilder->setOutTradeNo("201809181713");
    ...
    private function getAliPayObj(){
    	...
    	$aliPayObj->setAppId(ALI_PAY_APP_ID);
    	$aliPayObj->setCharset(ALI_PAY_CHARSET);
    	$aliPayObj->setMerchantPrivateKey(ALI_PAY_MERCHANT_PRIVATE_KEY);
    	$aliPayObj->setSignType(ALI_PAY_SIGN_TYPE);
    	$aliPayObj->setGateWayUrl(ALI_PAY_GATE_WAY_URL);
    	$aliPayObj->setAlipayPublicKey(ALI_PAY_ALIPAY_PUBLIC_KEY);
    	return $aliPayObj;
    }
    ...
    public function alipayReturn()
    {
        ...
        if (verifyAlipayReturn($configs)) {
            ...
            $dd = D('credit_charge')->where($map)->setField('status', 2);
            // var_dump($dd);exit;
            $this->assign('jumpUrl', U('public/Account/scoredetail'));
            $this->error('充值失败');
        }
