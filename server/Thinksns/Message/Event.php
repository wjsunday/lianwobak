...
class Event
{
    ...
    protected static function inputStatusMessage($client_id, array $message = array())
    {
        if (!empty($message['to_uid'])) {
            $users = $message['to_uid'];
        } else {
            $room = @Message::getRoomList($message['room_id']);
            if (!$room || empty($room[0]) || empty($room[0]['member_list'])) {
                return;
            }
            $users = \Lib\Util::arrayColumn($room[0]['member_list'], 'uid');
        }
        if ($users) {
            $client_ids = Message::getClientByUser($users, true);
            if ($client_ids) {
                $message = json_encode(array(
                    'type' => 'input_status',
                    'result' => array(
                        'room_id' => @(int) $message['room_id'],
                        'from_uid' => Message::getLoggedUserInfo('uid'),
                        'from_uname' => Message::getLoggedUserInfo('uname'),
                        'status' => empty($message['status']) ? false : true,
    ...
    {
        $room_id = isset($message['room_id']) ? $message['room_id'] : null;
        $operator = isset($message['operator']) ? $message['operator'] : null;
        $message_id = isset($message['message_id']) ? $message['message_id'] : null;
        $limit = isset($message['limit']) ? $message['limit'] : null;
        $data = Message::getMessageList($room_id, $operator, $message_id, $limit);
        if (is_int($data)) {
            $array = array(
                'Can\'t get the current user',
                'Current user not in the room',
    ...
    public static function clearMessageMessage($client_id, array $message = array())
    {
        ...
        if ($message['room_id'] && in_array($message['clear_type'], array('unread', 'all'))) {
            if (Message::clearMessage($message['room_id'], null, $message['clear_type'])) {
                self::sendDataToCurrentClient('clear_message', array(
                    'packid' => $message['packid'],
                    'room_id' => $message['room_id'],
                ));
    ...
    protected static function pushMessageDataToCurrent($data = null)
    {
        ...
        $data = array('from' => 'app', 'length' => count($data), 'list' => $data);
    ...
    protected static function pushMessageData($data, $users, $remove_current_user = true)
    {
        ...
        if ($client_ids) {
            foreach ($users as $key => $value) {
                $_data = $data[0];
                $_data['remark'] = $_data['remarks'][$value];
                unset($_data['remarks']);
                $message = json_encode(array(
                    'type' => 'push_message',
                    'result' => array(
                ...
                if(!isset($client_ids[$value])) continue;
                Gateway::sendToClient($client_ids[$value], $message);
            }
        ...
        if ($users) {
            unset($data['remarks']);
            $data = array_pop($data);
            $alert = $data['from_uname'].': '.$data['content'];
