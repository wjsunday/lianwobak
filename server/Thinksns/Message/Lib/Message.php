...
class Message
{
     ...
     * 创建/取得私聊房间号
     * @param  int       $uid 用户ID
     * @return array|int 如果成功返回包含room_id 的数组
     *                       如果失败则返回错误编号：
     *                       0：无法获取当前用户uid
     *                       1：聊天对象不存在
     *                       2：不能自己和自己聊天
            ...
            try {
                ...
                $result = self::sendMessage(array(
                    ...
                    'attach' => array(
                        ...
                        'room_member_num' => count($room['member_list']),
                    ),
                ), true);
                if (is_array($result)) {
                    return array(
    ...
    public static function quitGroupRoom($room_id)
    {
        try {
            ...
            //清除该成员消息
            self::clearMessage($room_id, $uid, 'all');
    ...
    public static function getRoomList($room_id, $mtime = null, $limit = null)
    {
        ...
        try {
            ...
            foreach ($result as $key => $rs) {
                ...
                $return[$key]['logo'] = intval($rs['logo']);
                // 取得全部用户的信息
                ...
                foreach ($members as $row) {
                    if (isset($users[$row['member_uid']])) {
                        ...
                        //备注
                        $member_list[$i]['remark'] = self::getRemark($uid, $row['member_uid']);
                ...
                if ($rs['last_message']) {
                    ...
                    if (is_array($last_message)) {
                        ...
                        if (isset($last_message['from_uid'])) {
                            ...
                            //备注
                            $return[$key]['last_message']['remark'] = self::getRemark($uid, $last_message['from_uid']);
                        } else {
                            $return[$key]['last_message']['from_uid'] = 0;
                            $return[$key]['last_message']['from_uname'] = '';
                            $return[$key]['last_message']['remark'] = '';
                        }
                    }
                }
    ...
    public static function sendMessage(array $message, $is_notify = false)
    {
        ...
        if ($message_id) {
            ...
            try {
                ...
                // 添加备注
                foreach ($to_uid_list as $key => $value) {
                    $return['remarks'][$value] = self::getRemark($value, $from_uid);
                }
    ...
    /**
     * 取得需要推送的消息列表
     * @param  null|int $uid 用户ID，为null时自动获取当前用户
     ...
     * 取得一个房间的消息列表
     * @param  int       $room_id    房间ID
     * @param  null|int  $message_id 一个消息ID，如果指定了此ID，那么返回数据列表为小于该ID的消息
     * @param  null|int  $limit      取多少条，设置小于0则为默认100条
     * @return array|int 如果失败返回一个数字代码，成功返回一个数组
     *                              0、无法获取当前用户信息
     *                              1、房间不存在或用户不在这个房间
     *                              2、系统错误
     */
    public static function getMessageList($room_id, $operator = null, $message_id = null, $limit = null)
    {
        $room_id = intval($room_id);
        ...
        if ($message_id > 0) {
            ...
            $where .= " AND `message_id`{$operator}{$message_id}";
            $sort = $operator == '<' || $operator == '<=' ? 'DESC' : 'ASC';
        } else {
            $sort = 'DESC';
        }
        ...
        $table = self::table('message_content');
        $sql = "SELECT * FROM `{$table}` WHERE {$where} ORDER BY `message_id` {$sort} LIMIT {$limit}";
        try {
            $result = self::db()->query($sql);
            if ($result) {
                return self::parseMessage($sort == 'DESC' ? array_reverse($result) : $result);
            }
    ...
    public static function parseMessage($list)
    {
        ...
        foreach ($list as $key => $rs) {
            ...
            //备注
            $uid = intval(self::getLoggedUserInfo('uid'));
            $array[$key]['remark'] = self::getRemark($uid, $rs['from_uid']);
     ...
     * @param null|int $uid     用户ID，null为当前用户
    ...
    /**
     * 检查一个用户是否在指定的房间
     * @param  int  $room_id    所在的房间ID
     * @param  int  $uid        需要检查的用户ID
    ...
    public static function getClientByUser($uid_list, $remove_current_user = true)
    {
        ...
        $where = "uid IN({$in_list}) AND gateway_id=".intval(GATEWAY_ID);
     ...
     * 移除一个用户到客户端的映射关系
     * @param null|int $uid       用户Id
     * @param null|int $client_id 客户端连接Id
     */
    public static function removeUserToClientMap($uid = null, $client_id = null)
    {
        $where = array('gateway_id='.intval(GATEWAY_ID));
        if ($uid > 0) {
            $where[] = 'uid='.(int) $uid;
        }
        if ($client_id > 0) {
     ...
     * @param  int $uid 当前查询用户id
     */
    public static function getRemark($mid, $uid)
    {
        $userremark = self::table('user_remark');

        $where = "`mid`={$mid} AND `uid`={$uid}";
        $sql = "SELECT * FROM `{$userremark}` WHERE {$where} limit 1";
        $remark = self::db()->query($sql);
        $rs = $remark ? $remark[0]['remark'] : '';
        return $rs;
    }
}
